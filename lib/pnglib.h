...
// 将 [0, 1] 映射到热量图，蓝色 -> 绿色 -> 黄色 -> 红色
RGB heatmap(double x){
    if(x < 0 || x > 1)
        return BLACK;
    if(x < 0.25)
        return (RGB){0, (unsigned char)(255 * x / 0.25), 255};
    if(x < 0.5)
        return (RGB){0, 255, (unsigned char)(255 * (0.5 - x) / 0.25)};
    if(x < 0.75)
        return (RGB){(unsigned char)(255 * (x - 0.5) / 0.25), 255, 0};
    return (RGB){255, (unsigned char)(255 * (1 - x) / 0.25), 0};
}

void pcolor(std::string fname, const Matrix &A){
    int n = A.nRows(), m = A.nCols();
    png img(m, n);
    double maxv = max(A), minv = min(A);
    for(int i = 0; i < n; i++)
        for(int j = 0; j < m; j++)
            img.setpix(m - 1 - j, i, heatmap((A(i, j) - minv) / (maxv - minv)));
    img.output(fname.c_str());
    std::cout << "Heatmap has been outputed to " << fname << std::endl;
}