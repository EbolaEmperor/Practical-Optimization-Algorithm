/********************************************************************************************************
 *
 * 这是一个DFP拟牛顿法的通用最优化程序，依赖于：matrix.h
 * 用法：sol=dfp(f,grad,n,x0,[err],[rho],[sigma])
...
Matrix dfp(double (*f)(const Matrix&), Matrix (*grad)(const Matrix&), const int &n, Matrix current, const double err=1e-5, const double rho=0.55, const double sigma=0.4){
    Matrix H = eye(n);
    int step = 0;
